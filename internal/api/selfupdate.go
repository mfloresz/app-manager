...
// resolveShell returns the absolute path to a POSIX-compatible shell suitable
// for running the generated self-update helper. On Termux /bin/sh does not
// exist; the shell lives under $PREFIX/bin/sh (typically
// /data/data/com.termux/files/usr/bin/sh). Order of preference:
//
//  1. $PREFIX/bin/sh when $PREFIX is set (Termux).
//  2. LookPath("sh") — covers /bin/sh, /usr/bin/sh, etc. on standard Linux.
//  3. /bin/sh as a last-resort literal, so the failure message points at a
//     missing file the operator can audit.
//
// Returns an error only when none of the above exist; otherwise the path is
// the one we will both embed as the script shebang and pass to exec.
func resolveShell() (string, error) {
	...
	return "", fmt.Errorf("no POSIX shell found: checked $PREFIX/bin/sh, PATH, and /bin/sh")
}
...
func (h *Handler) selfUpdate(w http.ResponseWriter, r *http.Request, repo *storage.Repository) {
	...
	// Resolve the shell *before* generating the script so the shebang matches
	// the interpreter the launcher will invoke (Termux: $PREFIX/bin/sh).
	shellPath, err := resolveShell()
	if err != nil {
		h.Broker.EmitError(repo.ID, "Error al localizar shell POSIX: "+err.Error())
		http.Error(w, "Error interno", http.StatusInternalServerError)
		return
	}
	scriptContent, logPath := generateSelfUpdateScript(binaryPath, pid, asset.DownloadURL, rel.TagName, mode, serviceName, shellPath)
...
//
// The shell used to invoke the helper is resolved at runtime via resolveShell
// so Termux (where /bin/sh does not exist) can run the update without the
// operator having to symlink it. The same resolved path is also embedded as
// the script's shebang (see generateSelfUpdateScript), so the file remains
// self-identifying if executed by hand.
func (h *Handler) launchSelfUpdate(scriptPath, unitName string, mode supervisorMode) error {
	...
	shellPath, err := resolveShell()
	if err != nil {
		return fmt.Errorf("auto-actualización: %w", err)
	}
	...
	if err := cmd.Start(); err != nil {
		return fmt.Errorf("no se pudo lanzar el script con %s: %w", shellPath, err)
	}
	return nil
}
...
//
// shellPath is the absolute path to the POSIX shell used to run the script;
// it is embedded verbatim as the shebang so the file is portable across
// Termux ($PREFIX/bin/sh), Linux (/bin/sh), and BSDs. The launcher uses the
// same resolved path to exec the script.
func generateSelfUpdateScript(binaryPath string, pid int, downloadURL, version string, mode supervisorMode, serviceName, shellPath string) (script string, logPath string) {
...
`, shellPath, shellQuote(binaryPath), pid, shellQuote(downloadURL), shellQuote(version), shellQuote(logPath), shellQuote(string(mode)), shellQuote(serviceName))
